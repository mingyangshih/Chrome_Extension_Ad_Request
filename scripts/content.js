    ...
    render_id_list.forEach( i => {
        ...
        if(checkSlotExist(i)) {
            ...
            if(rightnow - requestedTime > 60000) {
                ...
                if(document.getElementById(`console_${i}`).style.backgroundColor !== color.alert) {
                    document.getElementById(`console_${i}`).style.backgroundColor = color.alert
                }
            }else{
                ...
                if(checkSwapping(el,i)) {
                    document.getElementById(`console_${i}`).style.backgroundColor = color.swapping
                }else if(status === 'rendered' || status === 'dfpRendered' ||status === 'dfp_rendered') { //rendered
            ...
            //}
        }else{ // If slot do not in page remove list and console element.
            if(checkSlotExist(`console_${i}`)) {
...
function checkSwapping(el,id) {
    ...
    let ympb_id = document.getElementById(id).parentNode.parentNode.parentNode.dataset.ympbId
    ...
    //console.log(matrix)
    if(el.parentNode.style.transform !== 'none' && el.parentNode.style.transform) {
        //console.log(matrix)
...
function renderPositionToConsole(id) {
    ...
    let styleText = `width:${sizePercentageWidth * 250}px; height:25px; font-size:10px; background-color:${color.requested} `
    ...
    ympb_target.forEach( (i,idx) => {
        ...
        if(i.parentNode.className == 'ympb_target_multi') {
            if(!checkSlotExist(`console_multi_${i.parentNode.dataset.gid}`)) {
                ...
                let sizePercentageWidth = width / scrollWidth
                let styleText = `position: absolute; left: ${sizePercentageX * 250}px; top: ${sizePercentageY * 500}px`
                createNode(`console_multi_${i.parentNode.dataset.gid}`,`position_board`, styleText)
            }
            createNode(i.dataset.ympbId, `console_multi_${i.parentNode.dataset.gid}`,`margin-bottom: 5px;`)
...
function gptEventLogging() {
    ...
    googletag.cmd.push(function() {
        ...
        googletag.pubads().addEventListener('slotRequested', function(event) {
            ...
            if(slotId.indexOf('-oop') < 0) { //Ignore OOP slot
        ...
        googletag.pubads().addEventListener('slotOnload', function(event) {
            ...
            let bidder = advertiserList[event.slot.getResponseInformation().advertiserId.toString()]
            let advertiser = ( bidder || event.slot.getResponseInformation().advertiserId.toString());
